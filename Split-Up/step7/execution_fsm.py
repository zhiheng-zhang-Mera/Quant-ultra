...
重新修正（对齐最新要求）：全面恢复使用物理日历日（timedelta）执行历史价格因果追溯，禁止依赖交易日序号。
"""
import logging
import numpy as np
...
def get_prices_for_date(sym_list, date, data_bus, price_cache, lookback_days=10):
    """
    点时对齐价格追溯总线。当T日由于停牌无法拉取价格时，自适应向后平滑穿透lookback_days个物理日历日。
    刚性红线：采用物理日历日(timedelta)递减回溯，严禁转译为交易日序号。
    """
    prices = {}
    for sym in sym_list:
