...
def run_dsr_audit(context: dict) -> None:
    ...
    try:
        ...
        if len(nav_series) < min_samples:
            ...
            logger.warning("[操作] 评估时间序列样本规模 | [来源] 策略回测账本头部 | [结果] 有效行数不足: %s/%s | [意义] 自动降级挂起统计审计，防止小样本偏差导致虚假推断")
        ...
        if denom_sq <= 0:
            ...
            logger.warning("[操作] 求解共形方差分母 | [来源] 经验对数收益率分布 | [结果] 渐进方差值为非正数: %.4f | [意义] 检测到极端右尾异变噪声，触发防御性兜底保护")
            dsr_pval = 0.0
        else:
        ...
        logger.info("[操作] 终结 DSR 统计学检验推断 | [来源] 渐进正态拟合网格 | [结果] 夏普: %.4f, 试验次数: %s, DSR 显著性 p值: %.6f, 审查通过: %s | [意义] 断言策略真实的风险溢价回报是否稳健立于预设目标阈值之上")
        ...
        logger.error("[操作] 执行 DSR 审计算法 | [来源] 统计审计异常栈 | [结果] 崩溃异常: %s | [意义] 触发最严厉的风控一票否决，拒绝将本策略转交下游投产")
