...
def safe_get_shadow(context: Dict, key: str, default: Any = None) -> Any:
    ...
    if val is None and default is None:
        ...
        logger.critical("[操作] 拦截内存变量检索 | [来源] 流水线主注册上下文 | [结果] 核心依赖键缺失异常: %s | [意义] 强行挂起中断，严防下游因空指针引用造成计算链脱轨崩溃", key)
...
def compute_max_drawdown(nav: pd.Series) -> float:
    ...
    try:
        ...
        logger.error("[操作] 求解历历最大回撤 | [来源] 漂移净值时间序列 | [结果] 算术矩阵运算溢出: %s | [意义] 捕获非稳态数学不稳定性，自愈降级默认为零", str(e))
        return 0.0