    ...
    try:
        ...
        for scenario, (start_str, end_str) in stress_scenarios.items():
            ...
            if len(window_nav) < 2:
                ...
                logger.warning("[操作] 截取历史危机时间切片 | [来源] 极端场景日历轴: %s | [结果] 数据真空（零行或单行） | [意义] 当前回测窗口未覆盖该危机区间，安全跳过此项场景模拟")
                dd = float('nan')
            else:
                ...
                logger.info("[操作] 求解危机最大回撤 | [来源] 截取后的危机高波波时段: %s | [结果] 期间最大回撤: %.4f | [意义] 量化历史真实系统性尾部崩塌对本策略净值的最大压榨杀伤深度")
        ...
        logger.error("[操作] 运行极端高压模拟器 | [来源] 危机时空切轨管线 | [结果] 算法失败: %s | [意义] 保留默认空回撤字典注册表，向上游主控制器警示潜在的数据断层风险")
