...
def main() -> int:
    ...
    parser.add_argument("--archetype", default="", help="run only one archetype (balanced/defensive/momentum/safe/selector)")
    ...
    archetypes = ["balanced", *sorted(k for k in STRATEGY_ARCHETYPES if k != "balanced")]
    if args.archetype:
        archetypes = [a for a in archetypes if a == args.archetype]
    ...
    if not args.archetype or args.archetype == "selector":
    ...
    fixed_rows = [r for r in rows if r["archetype"] != "selector"]
    best_fixed = max(fixed_rows, key=lambda r: r["oos_sharpe"]) if fixed_rows else None
    sel_row = next((r for r in rows if r["archetype"] == "selector"), None)
    gate_pass = bool(
        sel_row is not None and best_fixed is not None
        and sel_row["oos_sharpe"] > best_fixed["oos_sharpe"] and sel_row["oos_ann"] > best_fixed["oos_ann"]
    )
    verdict = {
        "gate_pass": gate_pass,
        "recommendation": "ENABLE_selector" if gate_pass else "KEEP_disabled",
        "best_fixed": best_fixed["archetype"] if best_fixed else "",
        "reason": (
            f"selector OOS Sharpe {sel_row['oos_sharpe']:.3f} vs best fixed {best_fixed['archetype']} "
            f"{best_fixed['oos_sharpe']:.3f}; OOS ann {sel_row['oos_ann']:.2%} vs {best_fixed['oos_ann']:.2%}"
            if sel_row and best_fixed else "single-archetype run"
        ),
    }
