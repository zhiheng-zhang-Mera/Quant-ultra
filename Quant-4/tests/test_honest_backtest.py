...
def test_production_defaults_are_monthly_persistent_config():
    """The 2026-08-15 full-pool-revalidated production default: monthly
    rebalancing, position persistence, 12%/8% stop band, euphoria 0.15 and
    the adopted robust crash detection (fixed 3% + z-score 3.0). All
    leverage-free."""
    import sys
    from pathlib import Path
    ROOT = Path(__file__).parents[1]
    ...
    assert p.event_shock_threshold == 0.03
    assert p.event_shock_zscore == 3.0
    assert p.confirm_leverage == 1.0 and p.max_gross_exposure <= 1.0


def test_legacy_profile_preserves_old_defaults():
    """The --profile legacy overrides must restore the pre-2026-08-15 defaults
    for A/B comparison, without touching default_params()."""
    import sys
    from pathlib import Path
    ROOT = Path(__file__).parents[1]
    if str(ROOT) not in sys.path:
    ...
    p = default_params()
    # adopted production defaults are unchanged by the profile feature
    assert p.event_shock_threshold == 0.03
    assert p.event_shock_zscore == 3.0
    assert p.stop_loss_pct == 0.08
    # the legacy profile applies the documented pre-2026-08-15 overrides
    q = default_params()
    q.event_shock_threshold = 0.025
    q.event_shock_zscore = 0.0
    q.stop_loss_pct = 0.07
    assert q.event_shock_threshold == 0.025
    assert q.event_shock_zscore == 0.0
    assert q.stop_loss_pct == 0.07
