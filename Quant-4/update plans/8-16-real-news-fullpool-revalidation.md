...
**稀疏性诊断（为何方向反转无意义）**：38 只面板在 3 个引擎 rebalance 日期上，非零信号股票数
仅 **3 / 7 / 22 只**（免费源覆盖随日期增长但极稀疏），信号均值幅度 **0.005~0.034**（微弱）。
信号经 z-score + extra weight 0.10 进入复合打分，对 top-N 组合的实际扰动在噪声内——
方向反转（20 vs 38 只）是**稀疏微弱信号的偶然性**，不是可开采的方向。两组覆盖特征几乎相同
（n≈317-318 条/只、跨度 38.5 vs 33.1 天、情绪均值 0.057 vs 0.048），排除覆盖伪影。
