...
def recommendation(frame: pd.DataFrame, model_weight: float | None = None, max_weight: float = 0.10) -> dict:
    ...
    entry_low = max(0.01, anchor - 0.75 * snap["atr14"])
    entry_high = min(snap["last_price"] + 0.25 * snap["atr14"], snap["ma20"] + 0.50 * snap["atr14"])
    ...
    suggested_weight = float(np.clip(base_weight * confidence, 0.0, max_weight))
    take_profit_pct = float(np.clip(max(2.5 * snap["atr_pct"], 0.08), 0.08, 0.30))
    qualified = score >= 0 and suggested_weight > 0
    return {
        **perf, **snap, "score": score, "qualified": qualified,
        ...
        "formula_evidence": {
            "entry": "min(last, MA20) - 0.75*ATR14 至 min(last+0.25*ATR14, MA20+0.50*ATR14)",
            "allocation": "min(max_weight, model_weight * trend_confidence)",
            "take_profit": "clip(max(2.5*ATR14/last, 8%), 8%, 30%)",
        },
    }


def holding_advice(frame: pd.DataFrame, total_capital: float, quantity: int, average_cost: float, model_weight: float | None = None) -> dict:
    if total_capital <= 0 or quantity < 0 or average_cost <= 0:
        raise ValueError("总资金必须大于0、数量不得为负、平均成本必须大于0")
    rec = recommendation(frame, model_weight=model_weight)
    last = rec["last_price"]
