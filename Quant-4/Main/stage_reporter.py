"""Bilingual, self-verifying reports emitted for every pipeline phase."""
...
PHASE_GUIDE = {
    ...
    3: ("PIT 特征与另类数据", "PIT features and alternative data", "仅使用当时可见的价格、新闻、论坛情绪和资金池变化。", "Uses only prices, news, forum sentiment, and capital-pool changes available at the time."),
    ...
    5: ("模型训练与校准", "Model training and calibration", "训练方向与区间模型，并输出可复算的校准证据。", "Trains direction and interval models and emits reproducible calibration evidence."),
    ...
    7: ("交易状态机回测", "Execution FSM backtest", "模拟可交易性、整手成交、滑点、税费及账户净值。", "Simulates tradability, board lots, slippage, taxes, and account NAV."),
    ...
    9: ("影子对账与 MLOps", "Shadow reconciliation and MLOps", "比较目标和已执行仓位，监控漂移并激活交易门禁。", "Compares target and executed positions, monitors drift, and activates trading gates."),
    ...
    11: ("交互式投顾观察", "Interactive advisory observation", "生成候选建议；治理失败时禁止执行。", "Produces candidates; failed governance blocks execution."),
}
TERMS = {"PIT": "Point-in-Time / 时点可见信息", "NAV": "Net Asset Value / 账户净值", "MAE": "Mean Absolute Error / 平均绝对误差", "PSI": "Population Stability Index / 群体稳定性指数", "VaR": "Value at Risk / 风险价值", "CVaR": "Conditional Value at Risk / 条件风险价值", "ADV20": "20-day Average Daily Value / 20日平均成交额", "HOLD_FOR_REVIEW": "人工复核前禁止执行 / No execution before manual review"}

class StageReporter:
    def __init__(self, root: Path, run_id: str, git_hash: str):
        self.root = Path(root) / "runs" / run_id; self.root.mkdir(parents=True, exist_ok=True); self.git_hash, self.started = git_hash, {}
    ...
    def _describe(value: Any) -> dict:
        if isinstance(value, pd.DataFrame): return {"type": "DataFrame", "summary": f"shape={value.shape}, columns={list(value.columns)[:8]}, sha256={hashlib.sha256(value.to_csv(index=True).encode('utf-8')).hexdigest()[:16]}"}
