...
def _tier_blocks(proof: dict) -> str:
    """Render the three detail-level paragraphs for one phase proof."""
    levels = proof.get("detail_levels", {}) if isinstance(proof.get("detail_levels"), dict) else {}
    parts = []
    for tier, label in (("layman", "门外汉 / Layman"), ("beginner", "初学者 / Beginner"), ("expert", "专家 / Expert")):
        content = levels.get(tier) or {}
        zh = content.get("zh", "") if isinstance(content, dict) else ""
        en = content.get("en", "") if isinstance(content, dict) else ""
        parts.append(f"<div class='tier' data-tier='{tier}'><p>{_safe(zh)}</p><p>{_safe(en)}</p></div>")
    return "".join(parts)
...
def generate_execute_report(run_dir, context, requested_phases):
    ...
    for p in proofs:
        seconds=float(p.get("elapsed_seconds",0)); evidence="".join(f"<li><b>{_safe(_label(k))}</b>: {_safe(v.get('summary',''))}</li>" for k,v in list(p.get("outputs",{}).items())[:12]); cards.append(f"<article><h3>{_safe(p.get('title_zh',p.get('phase')))}<small>{_safe(p.get('title_en',''))}</small></h3>{_tier_blocks(p)}<div class='bar'><i style='width:{max(2,seconds/max_seconds*100):.1f}%'></i></div><small>{seconds:.3f}s · cache={_safe(p.get('cache_hit'))}</small><details><summary>输出证据 / Output evidence (专家细节 / expert detail)</summary><ul>{evidence}</ul></details></article>")
    ...
    layman_summary = ("报告显示系统已完成全部请求阶段的工程流程；具体投资结论请查看候选清单。"
                      if payload["passed_phases"] == payload["phase_count"] and payload["phase_count"] > 0
                      else "报告显示部分阶段未完成或治理门禁未通过，请先查看失败阶段与审计结论。")
    html_text=f"""<!doctype html><html lang='zh-CN'><head><meta charset='utf-8'><title>Quant-Ultra Execute Report</title><style>body{{background:#f5f7fb;color:#172033;font:15px system-ui;margin:0}}main{{max-width:1180px;margin:auto;padding:28px}}section,article{{background:#fff;border:1px solid #d9e0ea;border-radius:12px;padding:18px;margin:14px 0}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(330px,1fr));gap:14px}}small{{display:block;color:#667085}}table{{width:100%;border-collapse:collapse;display:block;overflow:auto}}th,td{{padding:8px;border-bottom:1px solid #d9e0ea;text-align:left;white-space:nowrap}}.bar{{height:7px;background:#d9e0ea}}.bar i{{display:block;height:100%;background:#2458d3}}.tier{{display:none}}body[data-tier='layman'] .tier[data-tier='layman']{{display:block}}body[data-tier='beginner'] .tier[data-tier='beginner']{{display:block}}body[data-tier='expert'] .tier[data-tier='expert']{{display:block}}.switch{{display:flex;gap:8px;margin:10px 0}}.switch button{{padding:8px 16px;border:1px solid #d9e0ea;border-radius:8px;background:#fff;cursor:pointer;font-size:14px}}.switch button.active{{background:#2458d3;color:#fff;border-color:#2458d3}}</style></head><body><main><section><h1>执行报告 / Execute Report</h1><div class='switch'><button data-set='layman'>门外汉 / Layman</button><button data-set='beginner'>初学者 / Beginner</button><button data-set='expert' class='active'>专家 / Expert</button></div><p data-tier-msg='layman'>{_safe(layman_summary)}</p><p data-tier-msg='beginner'><b>技术结论 / Technical conclusion:</b> {payload['passed_phases']}/{len(proofs)} 阶段通过；{_safe(conclusion)}。</p><p data-tier-msg='expert'><b>技术结论 / Technical conclusion:</b> {payload['passed_phases']}/{len(proofs)} 阶段通过；{_safe(conclusion)}。阶段完成只证明工程流程与合约状态；治理失败时不得执行候选交易。</p></section><section class='grid'><article>{_table({'final_nav':payload['final_nav'],'reconciliation_mae':payload['reconciliation_mae'],'audit_passed':payload['audit_passed'],'recon_passed':payload['recon_passed'],'cio_decision':payload['cio_decision'],'elapsed_seconds':elapsed})}</article><article>{_table(costs or {'costs':'NOT_AVAILABLE'})}</article></section><h2>计算环境 / Compute Environment</h2><section>{_table(compute.get('resource_plan',{}))}</section><h2>阶段证据 / Phase Evidence</h2><div class='grid'>{''.join(cards)}</div><h2>候选与四阶段主导方法 / Candidates and Dominant Methods</h2><section>{candidate_table}</section><section><h2>边界 / Limitations</h2><ul>{''.join(f'<li>{_safe(x)}</li>' for x in payload['limitations'])}</ul><small>Generated {_safe(payload['generated_at'])} · Git {_safe(payload['git_hash'])} · Data SHA-256 {digest}</small></section></main><script>const setLevel=l=>{{document.body.dataset.tier=l;document.querySelectorAll('.switch button').forEach(b=>b.classList.toggle('active',b.dataset.set===l));document.querySelectorAll('[data-tier-msg]').forEach(p=>p.style.display=(p.dataset.tierMsg===l)?'block':'none')}};document.querySelectorAll('.switch button').forEach(b=>b.addEventListener('click',()=>setLevel(b.dataset.set)));setLevel('expert');</script></body></html>"""
