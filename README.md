...
> **2026-08-16 真实另类信号复验轮（Round 16）**（详见 `Quant-4/update plans/8-16-real-news-fullpool-revalidation.md`）：网络诊断确认此前"无网络"为**误报**——PowerShell/curl 走 Windows Schannel 层被沙箱限制（`SEC_E_NO_CREDENTIALS`），而 **Python/OpenSSL 通道实际可用**（baidu/sina/pypi 及全部项目数据源 TLS/HTTP 均 200）。新工具 `Quant-4/tools/fetch_real_news_sina.py` 从新浪个股新闻页抓取生产池 20 只 **6352 条真实新闻**（契约全列、`is_synthetic=0`、sha256 入不可变缓存）。真实信号 A/B（采纳档基座）：通路方向响应正确——真实新闻情绪正权重单调拖累信号窗口（06-08 年化 3.26%→1.93%），**反转信号改善至 3.86%/0.52**——真实文本下结论与 R4/R11 一致：**新闻情绪不构成可用 alpha**；免费源仅约 5 个月窗口、28% 覆盖率，无法支撑全池历史复验，`alternative_signal_weight` 保持 0。测试套件 **175/175 全绿**。
...
- `Quant-4/update plans/8-16-real-news-fullpool-revalidation.md` — R16：真实另类信号复验（网络恢复，Schannel 误报诊断 + 真实新闻抓取 + 方向响应验证，新闻情绪无 alpha）
...
> **2026-08-16 real-data revalidation round (Round 16)** (see `Quant-4/update plans/8-16-real-news-fullpool-revalidation.md`): network diagnosis proved the earlier "no network" reports were **false negatives** — PowerShell/curl go through the Windows Schannel layer, which the sandbox restricts (`SEC_E_NO_CREDENTIALS`), while the **Python/OpenSSL path works** (baidu/sina/pypi and all project data-source hosts TLS/HTTP 200). New tool `Quant-4/tools/fetch_real_news_sina.py` pulled **6,352 real news items** for 20 production-pool names from Sina's per-stock news pages (full contract columns, `is_synthetic=0`, sha256 into the immutable cache). Real-signal A/B on the adopted base: the pathway's directional response is correct — positive news sentiment monotonically drags the signal window (06-08 ann 3.26%→1.93%) while the **inverted signal improves to 3.86%/0.52** — real text confirms the R4/R11 conclusion: **news sentiment is not usable alpha**; the free source covers only ~5 months at ~28% coverage, so full-history pool revalidation is data-constrained; `alternative_signal_weight` stays 0. **175/175 tests green**.
